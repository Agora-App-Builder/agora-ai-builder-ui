...
export const Index: Record<string, any> = {`
  for (const item of registry.items) {
    const resolveFiles = item.files?.map(
      (file) => `registry/agora-ui/${file.path}`
    )
    if (!resolveFiles) {
      continue
    }
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/agora-ui/${item.files[0].path}`
      : ""
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file) => {
      const filePath = `registry/agora-ui/${typeof file === "string" ? file : file.path}`
...
async function buildRegistryJsonFile() {
  // 1. Add registry/agora-ui prefix for the build to work
  const fixedRegistry = {
    ...registry,
    items: registry.items.map((item) => {
      const files = item.files?.map((file) => {
        return {
          ...file,
          path: `registry/agora-ui/${file.path}`,
        }
      })
  ...
  await new Promise((resolve, reject) => {
    exec(`prettier --write registry.json`, (error) => {
      if (error) {
        reject(error)
        return
      }
  ...
  return new Promise((resolve, reject) => {
    exec(
      ...
      async (error, stdout, stderr) => {
        ...
        // Post-process the generated JSON files to remove registry/agora-ui prefix
    ...
    if (json.files) {
      ...
      json.files.forEach((file: any) => {
        if (typeof file === "object" && file.path && file.target) {
          ...
          const sourcePath = file.path.replace(/^registry\/agora-ui\//, "")
      ...
      json.files = json.files.map((file: any) => {
        if (typeof file === "object" && file.path) {
          // Remove registry/agora-ui/ prefix
          let cleanPath = file.path.replace(/^registry\/agora-ui\//, "")
...
function rewriteImports(content: string, pathMappings: Map<string, string>): string {
  ...
  // Rewrite imports from @/registry/agora-ui/ui/... to @/components/ui/...
  content = content.replace(/@\/registry\/agora-ui\/ui\//g, "@/components/ui/")

  // Rewrite imports from @/registry/agora-ui/examples/... to @/components/examples/...
  content = content.replace(
    /@\/registry\/agora-ui\/examples\//g,
    "@/components/examples/"
  )

  // Rewrite imports from @/registry/agora-ui/lib/... to @/lib/...
  content = content.replace(/@\/registry\/agora-ui\/lib\//g, "@/lib/")
...
async function buildAllJson() {
  ...
  for (const component of uiComponents) {
    ...
    if (component.files) {
      for (const file of component.files) {
        const filePath = `registry/agora-ui/${file.path}`
        ...
        // Clean path - remove registry/agora-ui/ prefix
  ...
  const allJson = {
    ...
    name: "all",
    type: "registry:ui",
    description: "All UI components from Agora AI Builder UI",
